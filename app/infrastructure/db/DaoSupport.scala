...
import domain.supoport.{EntityNotFoundException, RepositoryIOException}
...
trait DaoSupport[T] {
  ...
  def insertOrUpdate(id: Long, version: Option[Long], record: T)(implicit s: DBSession): Try[T] = Try {
    if (version.isDefined) {
      ...
      val count = mapper.updateBy(where).withAttributes(mapper.toNamedValues(record).filterNot {
